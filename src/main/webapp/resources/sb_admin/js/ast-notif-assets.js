...
(function( root, factory ) {
	if( typeof define === 'function' && define.amd ) {
		...
		define( function() {
			root.AstComeif = factory();
			return root.AstComeif;
		} );
	...
	var dialogState = {
		...
		callbackPositive: function () {
			if(stockbondCheck=='stock') {
				deleteStock();
			}
			if(stockbondCheck=='bond') {
				deleteBond();
			}		
		},
		...
		show: function() {
			...
			var $AstComeifThis = this; // Passing this to another inner function
			...
			if (this.options.lebayify > 0) {
				...
				setTimeout(function() {
					typewriterEffect(textElement, $AstComeifThis.message);
				}, 200);
			}
			...
			dialogElement.addEventListener('click', function(e) {
			    if (e.target && e.target.id == 'ast-positive-dialog-button') {
					$AstComeifThis.callbackPositive();
					$AstComeifThis.close();
			    }

			    if (e.target && e.target.id == 'ast-negative-dialog-button') {
					$AstComeifThis.callbackNegative();
					$AstComeifThis.close();
			    }
			});
		}
	...
	var notifyState = {
		...
		inited: function() {
			var AstComeifyElement = document.getElementById("ast-notify-container");
			if (typeof AstComeifyElement !== "undefined" && AstComeifyElement != null) {
				return AstComeifyElement;
			}
			else
				return false;
		},
	...
	return (window.AstComeif = window.astn = AstComeif);
}));